...
`raxmlHPC-PTHREADS-AVX -h` or die "\nERROR: Cannot find raxmlHPC-PTHREADS-AVX in \$PATH\n\n";
...
$pm->start and next DATA_LOOP;
	...
	LOOP: while (my $fas = shift @$array) {
		...
		# construct tree
		`raxmlHPC-PTHREADS-AVX -p 12345 -m GTRGAMMA -s $copy_path -n $gene.tre`; # run raxml
...
print "######### Start Constrained ML tree ########\n";
	...
	$pm->start and next DATA_LOOP;
		...
		my ($gene) = $fas =~ /(\S+)\.fa\S*$/;
			...
			`raxmlHPC-PTHREADS-AVX -p 12345 -m GTRGAMMA -s $path -g $constrain_tree_path -n $gene.tre`;
...
Dependencies:
(1) raxmlHPC-PTHREADS-AVX (download from 'https://github.com/stamatak/standard-RAxML')
(2) Perl module:
